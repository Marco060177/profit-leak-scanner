...
export type ProfitAlert = {
   ...
   * Rimane numerico per poter essere formattato dalla UI.
   */
  monthlyImpact: number;

  /**
   * Priorità interna da 0 a 100.
   * Più è alta, più l'alert viene mostrato in alto.
   */
  priority: number;
  ...
  /**
   * Nome del prodotto collegato all'alert, quando disponibile.
   */
  productTitle?: string;

  /**
   * ID Shopify del prodotto, quando disponibile.
   */
  productId?: string;

  /**
   * Dati aggiuntivi utilizzabili dalle interfacce future.
   */
  metadata?: {
...
type GenerateProfitAlertsInput = {
  ...
  rows: ProfitMonitorRow[];
  language: Language;

  /**
   * Periodo dei dati caricati.
   * Serve per trasformare gli importi del periodo in stime mensili.
   *
   * Esempi:
   * "7"
   * "30"
...
const severityWeight: Record<ProfitAlertSeverity, number> = {
  critical: 4,
  warning: 3,
  opportunity: 2,
  info: 1,
